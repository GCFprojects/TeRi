...
def writeToFile(tab, excelResult=None, param=None):
    if param == 'Result':
        ...
        fail = open(dir_path+'\\Fails_to _user_veryfication.txt', 'a', encoding='utf-8')
        for item in tab:
...
def checkResults(paramList):
    resultFail = [x for x in paramList if x[2] == 'FAIL' or x[2] == 'INCONCLUSIVE' or x[2] == 'ERROR' or x[2] == 'INVALID']
