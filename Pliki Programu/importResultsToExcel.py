            ...
            if str(sheet.cell_value(item, 0)) == tcList[0]:
                ...
                if help_tryexc == 'First':
                    ...
                    if sheet.cell_value(item, 8) == '' and sheet.cell_value(item, 9) == '':
                        ...
                        rb_sheet.write(item, 9, tcList[4][1:], styleTimeCell)
                    ...
                    for i in range(100):
                        ...
                        if flag:
                            ...
                            if sheet.cell_value(item+i, 8) == '' and sheet.cell_value(item+i, 9) == '':
                                # Jeżeli są puste wprowadź wyniki do excel`a
                                rb_sheet.write(item+i, 8, tcList[2][0], styleResultCell)
                                rb_sheet.write(item+i, 9, tcList[4][1:], styleTimeCell)
                                rb_sheet.write(item+i, 10, moduleNumber, styleTestRunCell)
...
def sheetFunctions(excelSheetName, sheet, rb_sheet):
    ...
    if excelSheetName == '2G':
        ...
        rb_sheet.write(cellNumber['TC'] + 7, 9, xlwt.Formula('SUM(J' + str(TC['KC208'] + 2) + ':J' + str(TC['KC211']) + ')'), styleTimeCell)
