...
def sheetFunctions(excelSheetName, sheet, rb_sheet):
    ...
    styleSummeTimeCell = easyxf('align: vert centre, horiz centre',  num_format_str='H:MM:SS')
    styleNACell = easyxf('font: color blue; align: vert centre, horiz centre')
    styleTimeCell = easyxf('align: vert centre, horiz centre; borders: left double, right double, top double, bottom  double', num_format_str='H:MM:SS')

    cellNumber = {'NA':0, 'Pass':0, 'Fail':0, 'NotExecuted':0, 'Summe':0, 'TC':0}
    tab = []
    if excelSheetName == '2G':
        TC = {'KC201':'', 'KC202':'', 'KC203':'', 'KC204':'', 'KC205':'', 'KC206':'', 'KC207':'', 'KC208':'',
              'KC221':'', 'KC222':'', 'KC223':'', 'KC231':'', 'KC233':'', 'KC241':''}
    ...
    if cellNumber['NA'] != '' and cellNumber['Pass'] != '' and cellNumber['Fail'] != '' and cellNumber['NotExecuted'] != '' and cellNumber['Summe'] != '':
        rb_sheet.write(cellNumber['NA'], 8, Formula('COUNTIF(I3:I'+str(cellNumberMin-1)+';"=N/A")'),styleNACell )
        rb_sheet.write(cellNumber['Pass'], 8, Formula('COUNTIF(I3:I'+str(cellNumberMin-1)+';"=P")'),stylePassedCell)
        rb_sheet.write(cellNumber['Fail'], 8, Formula('COUNTIF(I3:I'+str(cellNumberMin-1)+';"=F")+COUNTIF(I4:I'+str(cellNumberMin-1)+';"=I")'),styleFailedCell)
        rb_sheet.write(cellNumber['NotExecuted'], 8, Formula('COUNTIF(I3:I'+str(cellNumberMin-1)+';"")'),styleNotExecutedCell)
        rb_sheet.write(cellNumber['Summe'], 8, Formula('SUM(I'+str(cellNumberMin+1)+':I'+str(cellNumber['Summe'])+')'),styleSummeCell)
        rb_sheet.write(cellNumber['Summe'], 9, Formula('SUM(J3:J'+str(cellNumberMin-1)+')'), styleSummeTimeCell)
    ...
    if excelSheetName == '2G':
        rb_sheet.write(cellNumber['TC'], 9, Formula('SUM(J' + str(TC['KC201'] + 2) + ':J' + str(TC['KC202']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 1, 9, Formula('SUM(J' + str(TC['KC202'] + 2) + ':J' + str(TC['KC203']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 2, 9, Formula('SUM(J' + str(TC['KC203'] + 2) + ':J' + str(TC['KC204']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 3, 9, Formula('SUM(J' + str(TC['KC204'] + 2) + ':J' + str(TC['KC205']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 4, 9, Formula('SUM(J' + str(TC['KC205'] + 2) + ':J' + str(TC['KC206']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 5, 9, Formula('SUM(J' + str(TC['KC206'] + 2) + ':J' + str(TC['KC207']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 6, 9, Formula('SUM(J' + str(TC['KC207'] + 2) + ':J' + str(TC['KC208']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 7, 9, Formula('SUM(J' + str(TC['KC208'] + 2) + ':J' + str(TC['KC221']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 8, 9, Formula('SUM(J' + str(TC['KC221'] + 2) + ':J' + str(TC['KC222']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 9, 9, Formula('SUM(J' + str(TC['KC222'] + 2) + ':J' + str(TC['KC223']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 10, 9, Formula('SUM(J' + str(TC['KC223'] + 2) + ':J' + str(TC['KC231']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 11, 9, Formula('SUM(J' + str(TC['KC231'] + 2) + ':J' + str(TC['KC233']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 12, 9, Formula('SUM(J' + str(TC['KC233'] + 2) + ':J' + str(TC['KC241']) + ')'), styleTimeCell)
        rb_sheet.write(cellNumber['TC'] + 13, 9, Formula('SUM(J' + str(TC['KC241'] + 2) + ':J' + str(cellNumberMin - 1) + ')'), styleTimeCell)
    elif excelSheetName == '3G':
        rb_sheet.write(cellNumber['TC'], 9, Formula('SUM(J' + str(TC['KC401'] + 2) + ':J' + str(TC['KC402']) + ')'), styleTimeCell) # WCDMA R99
        rb_sheet.write(cellNumber['TC'] + 3, 9, Formula('SUM(J' + str(TC['KC404'] + 2) + ':J' + str(TC['KC405']) + ')'), styleTimeCell) # WCDMA R5 HSDPA
        rb_sheet.write(cellNumber['TC'] + 4, 9, Formula('SUM(J' + str(TC['KC405'] + 2) + ':J' + str(TC['KC406']) + ')'), styleTimeCell) # WCDMA R6 HSUPA
        rb_sheet.write(cellNumber['TC'] + 6, 9, Formula('SUM(J' + str(TC['KC420'] + 2) + ':J' + str(TC['KC421']) + ')'), styleTimeCell) # WCDMA R7 CIPH
        rb_sheet.write(cellNumber['TC'] + 7, 9, Formula('SUM(J' + str(TC['KC421'] + 2) + ':J' + str(TC['KC422']) + ')'), styleTimeCell) # WCDMA R7 CPC
        rb_sheet.write(cellNumber['TC'] + 8, 9, Formula('SUM(J' + str(TC['KC422'] + 2) + ':J' + str(TC['KC423']) + ')'), styleTimeCell) # WCDMA R7 64QAM
        rb_sheet.write(cellNumber['TC'] + 9, 9, Formula('SUM(J' + str(TC['KC423'] + 2) + ':J' + str(TC['KC424']) + ')'), styleTimeCell) # WCDMA R7 L2ENH
        rb_sheet.write(cellNumber['TC'] + 10, 9, Formula('SUM(J' + str(TC['KC424'] + 2) + ':J' + str(TC['KC425']) + ')'), styleTimeCell) # WCDMA ECFACH DL
        rb_sheet.write(cellNumber['TC'] + 11, 9, Formula('SUM(J' + str(TC['KC425'] + 2) + ':J' + str(TC['KC426']) + ')'), styleTimeCell) # WCDMA R7 MIMO
        rb_sheet.write(cellNumber['TC'] + 12, 9, Formula('SUM(J' + str(TC['KC426'] + 2) + ':J' + str(TC['KC427']) + ')'), styleTimeCell) # WCDMA R8 CSVoH
...
def writeToExcel(rb_sheet, item, tcList, moduleNumber, excelStyle, location):
    ...
    rb_sheet.write(item, 9, datetime.datetime.strptime(tcList[4], "%H:%M:%S").time(), excelStyle[1])
